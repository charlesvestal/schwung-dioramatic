...
typedef struct {
    /* 8 musical knobs (0-1) */
    float space;     /* reverb size/mode + send amount */
    float shimmer;   /* octave-up shimmer grains (Haze C behavior) */
    float smear;     /* micro-grain cloud density (Haze A behavior) */
    float warmth;    /* filter cutoff (inverted) */
    float drift;     /* pitch mod + reverb modulation */
    float sustain;   /* grain amplitude + reverb feedback */
    float scatter;   /* mosaic cascading loops + blocks sparkle events */
    float mix;       /* dry/wet */

    /* Internal params derived from knobs (set in process_block) */
    int algorithm;       /* unused but kept for grain functions */
    int variation;       /* unused but kept for grain functions */
    int time_div;
    int reverb_mode;
    int reverse;
    int hold;
...
/* Unified tick: blends multiple grain behaviors based on 8 musical knobs.
...
static void *v2_create_instance(const char *module_dir, const char *config_json) {
    ...
    /* Internal defaults (overwritten by knob mapping each block) */
    inst->algorithm = 0;
    inst->variation = 0;
    inst->activity = 0.5f;
    inst->repeats = 0.5f;
...
static void v2_process_block(void *instance, int16_t *audio_inout, int frames) {
    ...
    /* === MAP 8 MUSICAL KNOBS TO INTERNAL PARAMS ===
    ...
    inst->filter_res = inst->warmth * 0.3f;

    /* Sustain → repeats (grain amplitude/duration) */
    inst->repeats = inst->sustain;

    /* Drift → pitch mod */
    inst->pitch_mod_depth = inst->drift * 0.2f;
    inst->pitch_mod_rate = 0.1f + inst->drift * 0.3f;

    /* Internal defaults */
    inst->time_div = 2;  /* 1x */
    inst->reverse = 0;
    inst->hold = 0;
    inst->shape = 1.0f;  /* shape LFO off */
...
static int v2_get_param(void *instance, const char *key, char *buf, int buf_len) {
    ...
    if (strcmp(key, "space") == 0) return snprintf(buf, buf_len, "%.3f", inst->space);
    if (strcmp(key, "shimmer") == 0) return snprintf(buf, buf_len, "%.3f", inst->shimmer);
    if (strcmp(key, "smear") == 0) return snprintf(buf, buf_len, "%.3f", inst->smear);
    if (strcmp(key, "warmth") == 0) return snprintf(buf, buf_len, "%.3f", inst->warmth);
    if (strcmp(key, "drift") == 0) return snprintf(buf, buf_len, "%.3f", inst->drift);
    if (strcmp(key, "sustain") == 0) return snprintf(buf, buf_len, "%.3f", inst->sustain);
    if (strcmp(key, "scatter") == 0) return snprintf(buf, buf_len, "%.3f", inst->scatter);
