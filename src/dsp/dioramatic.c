...
static void fdn_process(fdn_reverb_t *rev, float in_l, float in_r,
    ...
    for (int i = 0; i < FDN_LINES; i++) {
        ...
        /* Soft limit — only engages on large signals, transparent otherwise */
        if (fb > 0.7f || fb < -0.7f) fb = tanhf(fb) * 0.98f;
...
static void v2_process_block(void *instance, int16_t *audio_inout, int frames) {
    ...
    /* Exponential curve so high sustain approaches unity for long tails */
    float rev_feedback = 1.0f - 0.35f * powf(1.0f - inst->sustain, 2.0f);
    if (rev_feedback > 0.998f) rev_feedback = 0.998f;
    ...
    for (int i = 0; i < frames; i++) {
        ...
        if (rev_send > 0.01f) {
            fdn_process(&inst->reverb, (dry_l + wet_l) * rev_send, (dry_r + wet_r) * rev_send,
